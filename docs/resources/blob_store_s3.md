...
**S3 Pre-Signed URLs**: The `pre_signed_url_enabled` attribute is only supported on Sonatype Nexus Repository Manager (NXRM) 3.79.0 PRO and later. This attribute will have no effect on earlier versions.
...
- `pre_signed_url_enabled` (Boolean) Whether pre-signed URL is enabled or not. Defaults to `false`. **Requires NXRM 3.79.0 PRO or later**
